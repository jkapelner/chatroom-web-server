...
class Model_User extends \Orm\Model
{
  ...
  /**
   * User's virtual password, used for validation purposes
   *
  ...
  {
    static::_add_feature_properties();
  }
   ...
   *
   * @throws \Warden\Failure If the user needs to be confirmed
   */
  public static function authenticate($username_or_email_or_id, $force = false, $check_confirmation = true)
  {
    if (empty($username_or_email_or_id)) {
        return null;
    }

  	if ($username_or_email_or_id == (string)(integer)$username_or_email_or_id) {
...
SQL;
  ...
  public static function anonymous()
  {
    ...
    $user    || $user    = $session->get('anonymous_user');
    ...
    $user->email    = "{$persistence_token}@warden-auto-generated-me.com";
  ...
  {
    return (bool)(preg_match('/@warden-auto-generated-me.com$/', $this->email) === 1);
  }
   ...
   * @return bool
   *
   * @throws \Warden\Failure If the user is already confirmed
   */
   ...
   *                                 or null if no user is found.
   *
   * @throws \Warden\Failure If the token has expired or the user is not locked
   */
   ...
   *
   * @throws \Warden\Failure If attempts exceeded
   */
  ...
  private function _username_or_email_exists()
  {
    ...
    if ($this->is_changed('email')) {
        $sql = "(SELECT email FROM $table WHERE email = $email)";
        
        if ($this->is_changed('username')) {
            $sql .= " UNION (SELECT email FROM $table WHERE username = $username)";
        }
    }
    else {
  ...
  /**
   * Dynamically adds properties for features that are enabled.
   *
   * @see \Warden\Model_User::_init()
   */
  private static function _add_feature_properties()
  {
    static::_init_rememberable();
  ...
  /**
   * Dynamically adds properties to \Warden\Model_User::$_properties.
   *
  ...
  /**
   * Checks that the rememberable feature is enabled and adds its required
  ...
  private static function _init_omniauthable()
  {
    ...
    public static function get_users_with_role($role_id, $limit = 0)
    {
        $query = \DB::select('user_id')->from('roles_users')->where('role_id', '=', $role_id);
    ...
    public static function count_users_with_role($role_id)
    {
        $result = \DB::query("SELECT COUNT(*) AS count FROM roles_users WHERE role_id = $role_id")->execute();
        ...
        return (integer)$result_array[0]['count'];
    }
}
